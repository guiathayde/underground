...
Ranking::Ranking(GraphicManager *graphicManager) : Menu(graphicManager)
{
  ...
  for (int i = 1; i < RANK_MAX_SIZE; i++)
  {
    ...
    name[i].setPosition(windowSize.x / 3.0f, i * 40.0f);
    ...
    score[i].setPosition(windowSize.x / 2.0f, i * 40.0f);
  }
}
...
void Ranking::Draw(sf::RenderWindow *window, sf::View *view)
{
  ...
  map<const char *, int>::iterator it = ranking.begin();
...
void Ranking::InsertRank(const char *name, int score)
{
  ranking.emplace(name, score);
  sizeWithRank++;
}

void Ranking::SetRank(const char *name, int score)
{
